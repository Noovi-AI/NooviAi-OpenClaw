...
async function promptConfiguredAction(params: {
  ...
  const deleteOption: WizardSelectOption<ConfiguredChannelAction> = {
    value: "delete",
    label: t("onboard.channels.deleteConfig"),
  };
  const skipOption: WizardSelectOption<ConfiguredChannelAction> = {
    value: "skip",
    label: t("onboard.channels.skipLeaveAsIs"),
  };
...
async function promptRemovalAccountId(params: {
  ...
  const selected = await prompter.select({
    message: t("onboard.channels.accountSelect", { label }),
...
async function collectChannelStatus(params: {
  ...
  const catalogStatuses = catalogEntries.map((entry) => ({
    ...
    selectionHint: `plugin · ${t("common.install").toLowerCase()}`,
    quickstartScore: 0,
  }));
...
async function maybeConfigureDmPolicies(params: {
  ...
  const wants = await prompter.confirm({
    message: t("onboard.channels.configureDmNow"),
    initialValue: false,
  });
  if (!wants) {
    return params.cfg;
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    ...
    return (await prompter.select({
      ...
      options: [
        { value: "pairing", label: t("onboard.channels.policyPairing") },
        { value: "allowlist", label: t("onboard.channels.policyAllowlist") },
        { value: "open", label: t("onboard.channels.policyOpen") },
        { value: "disabled", label: t("onboard.channels.policyDisabled") },
      ],
    })) as DmPolicy;
  };
...
export async function setupChannels(
  ...
  const shouldConfigure = options?.skipConfirm
    ...
    : await prompter.confirm({
        message: t("onboard.channels.configureNow"),
        initialValue: true,
      });
  if (!shouldConfigure) {
    return cfg;
  ...
  const configureChannel = async (channel: ChannelChoice) => {
    ...
    if (!adapter) {
      await prompter.note(
        t("onboard.channels.noOnboarding", { channel }),
        t("onboard.channels.setupLabel"),
      );
      return;
    }
  ...
  const handleConfiguredChannel = async (channel: ChannelChoice, label: string) => {
    ...
    if (action === "delete" && !supportsDelete) {
      await prompter.note(
        t("onboard.channels.noDeleteSupport", { label }),
        t("onboard.channels.removeLabel"),
      );
      return;
    }
    ...
    if (action === "delete") {
      const confirmed = await prompter.confirm({
        message: t("onboard.channels.deleteAccount", { label, account: accountLabel }),
        initialValue: false,
      });
      if (!confirmed) {
        return;
  ...
  if (options?.quickstartDefaults) {
    ...
    const choice = (await prompter.select({
      message: t("onboard.channels.selectQuickstart"),
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: t("onboard.channels.skipForNow"),
          hint: t("onboard.channels.addLater", {
    ...
    while (true) {
      ...
      const choice = (await prompter.select({
        ...
        options: [
          ...
          {
            value: doneValue,
            label: t("onboard.channels.finished"),
            hint:
              selection.length > 0 ? t("onboard.channels.done") : t("onboard.channels.skipForNow"),
          },
        ],
        initialValue,
  ...
  if (selectedLines.length > 0) {
    await prompter.note(selectedLines.join("\n"), t("onboard.channels.selectedLabel"));
  }
