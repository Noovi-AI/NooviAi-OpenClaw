    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("common.loadingShort") : t("common.refresh")}
        </button>
      </div>
      ${
        props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          pending.length > 0
            ? html`
              <div class="muted" style="margin-bottom: 8px;">${t("nodes.pendingLabel")}</div>
        ...
        ${
          paired.length > 0
            ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${t("nodes.pairedLabel")}</div>
        ...
        ${
          tokens.length === 0
            ? html`
              ...
              `
            : html`
              <div class="muted" style="margin-top: 10px;">${t("nodes.tokensLabel")}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodes.revoked") : t("nodes.activeToken");
...
function getSecurityOptions(): Array<{ value: ExecSecurity; label: string }> {
  return [
    { value: "deny", label: t("nodes.denyOption") },
    { value: "allowlist", label: t("nodes.allowlistOption") },
    { value: "full", label: t("nodes.fullOption") },
  ];
}
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          ...
          <div class="card-sub">
            ${t("nodes.execNodeBindingSub")} <span class="mono">exec host=node</span>.
          </div>
        </div>
        <button
          class="btn"
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodes.loadConfigBindings")}</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t("common.loadingShort") : t("nodes.loadConfig")}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                ...
                <div class="list-meta">
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodes.noNodesAvailable")}</div>
                        `
                      : nothing
                  }
                </div>
              ...
              ${
                state.agents.length === 0
                  ? html`
                      <div class="muted">${t("nodes.noAgentsFound")}</div>
                    `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          ...
          <div class="card-sub">
            ${t("nodes.execApprovalsSub")} <span class="mono">exec host=gateway/node</span>.
          </div>
        </div>
        <button
          class="btn"
        ...
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? t("common.loadingShort") : t("nodes.loadApprovals")}
            </button>
          </div>`
          : html`
    ...
    <div class="list" style="margin-top: 12px;">
      ...
      ${
        state.target === "node" && !hasNodes
          ? html`
              <div class="muted">${t("nodes.noNodesApprovals")}</div>
            `
          : nothing
      }
    </div>
    ...
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? t("nodes.defaultSecurityMode") : t("nodes.defaultValue", { value: defaults.security })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t("nodes.useDefaultValue", { value: defaults.security })}
                  </option>`
                  : nothing
              }
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? t("nodes.defaultPromptPolicy") : t("nodes.defaultValue", { value: defaults.ask })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${t("nodes.useDefaultValue", { value: defaults.ask })}
                  </option>`
                  : nothing
              }
              ${getAskOptions().map(
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodes.askFallbackSub")
                : t("nodes.defaultValue", { value: defaults.askFallback })
            }
          </div>
        </div>
        <div class="list-meta">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${t("nodes.useDefaultValue", { value: defaults.askFallback })}
                  </option>`
                  : nothing
              }
              ${getSecurityOptions().map(
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${
              isDefaults
                ? t("nodes.autoAllowSkillsSub")
                : autoIsDefault
                  ? t("nodes.usingDefault", { value: defaults.autoAllowSkills ? "on" : "off" })
                  : t("nodes.overrideStatus", { value: autoEffective ? "on" : "off" })
            }
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes.enabledLabel")}</span>
            <input
              type="checkbox"
              ?disabled=${state.disabled}
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        ...
        <div class="card-sub">${t("nodes.globPatterns")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list" style="margin-top: 12px;">
      ${
        entries.length === 0
          ? html`
              <div class="muted">${t("nodes.noAllowlistEntries")}</div>
            `
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? t("nodes.defaultAgent") : t("nodes.agentLabel")} ·
          ${
            bindingValue === "__default__"
              ? t("nodes.usesDefault", { value: state.defaultBinding ?? t("nodes.anyNode") })
              : t("nodes.overrideValue", { value: agent.binding ?? "" })
          }
        </div>
      </div>
      <div class="list-meta">
      ...
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          ...
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodes.connectedChip") : t("nodes.offline")}
          </span>
