...
async function promptIMessageAllowFrom(params: {
  ...
  await params.prompter.note(
    t("channelOnboarding.imessage.allowlistHelp", {
      docsLink: formatDocsLink("/imessage", "imessage"),
    }),
    t("channelOnboarding.imessage.allowlistLabel"),
  );
  const entry = await params.prompter.text({
    message: t("channelOnboarding.imessage.allowFromPrompt"),
    placeholder: t("channelOnboarding.imessage.allowFromPlaceholder"),
    ...
    validate: (value) => {
      ...
      for (const part of parts) {
        ...
        if (part.toLowerCase().startsWith("chat_id:")) {
          ...
          if (!/^\d+$/.test(id)) {
            return t("channelOnboarding.imessage.invalidChatId", { entry: part });
          }
          continue;
        }
        if (part.toLowerCase().startsWith("chat_guid:")) {
          if (!part.slice("chat_guid:".length).trim()) {
            return t("channelOnboarding.imessage.invalidChatGuid");
          }
          continue;
        }
        if (part.toLowerCase().startsWith("chat_identifier:")) {
          if (!part.slice("chat_identifier:".length).trim()) {
            return t("channelOnboarding.imessage.invalidChatIdentifier");
          }
          continue;
        }
        if (!normalizeIMessageHandle(part)) {
          return t("channelOnboarding.imessage.invalidHandle", { entry: part });
        }
      }
      return undefined;
    },
...
export const imessageOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  getStatus: async ({ cfg }) => {
    ...
    return {
      ...
      statusLines: [
        configured
          ? t("channelOnboarding.imessage.statusConfigured")
          : t("channelOnboarding.imessage.statusNeedsSetup"),
        `${imessageCliDetected ? t("channelOnboarding.imessage.cliFound") : t("channelOnboarding.imessage.cliMissing")} (${imessageCliPath})`,
      ],
      selectionHint: imessageCliDetected
        ? t("channelOnboarding.imessage.cliFound")
        : t("channelOnboarding.imessage.cliMissing"),
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
    ...
    if (!cliDetected) {
      const entered = await prompter.text({
        message: t("channelOnboarding.imessage.cliPathPrompt"),
      ...
      if (!resolvedCliPath) {
        await prompter.note(t("channelOnboarding.imessage.cliPathRequired"), "iMessage");
      }
    }
    ...
    await prompter.note(
      t("channelOnboarding.imessage.nextStepsHelp", {
        docsLink: formatDocsLink("/imessage", "imessage"),
      }),
      t("channelOnboarding.imessage.nextStepsLabel"),
    );
